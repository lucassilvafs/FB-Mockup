...
export default function SelectCustom({ inicialProduct, selected, setSelected, products }) {
  return (
    <Listbox value={selected ? selected : inicialProduct} onChange={setSelected}>
      <div className="relative mt-2">
        <ListboxButton className="grid w-full cursor-default grid-cols-1 rounded-md bg-white py-1.5 pr-2 pl-3 text-left text-gray-900 outline-1 -outline-offset-1 outline-gray-300 focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-indigo-600 sm:text-sm/6">
          <span className="col-start-1 row-start-1 flex items-center gap-3 pr-6">
            <img alt="" src={selected ? selected : inicialProduct.icon} className="size-5 shrink-0 rounded-full bg-gray-100" />
            <span className="block truncate">{selected ? selected.name : inicialProduct.name}</span>
          </span>
          <ChevronUpDownIcon
            aria-hidden="true"
          ...
          {products.map((product) => (
            ...
            >
              <div className="flex items-center">
                <img alt="" src={product.icon} className="size-5 shrink-0 rounded-full" />
                <span className="ml-3 block truncate font-normal group-data-selected:font-semibold">{product.name}</span>
              </div>
